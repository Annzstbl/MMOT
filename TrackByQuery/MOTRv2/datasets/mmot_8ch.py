...
from mmot.mmlab.hs_mmrotate import poly2obb, poly2obb_np
...
from mmot.datasets.pipelines.compose import MotCompose, MotRandomChoice
from mmot.datasets.pipelines.channel import MotrToMmrotate, MmrotateToMotrv2
from mmot.datasets.pipelines.loading import MotLoadAnnotations, MotLoadImageFromFile, MotRLoadProposals, MotLoadMultichannelImageFromNpy
from mmot.datasets.pipelines.transforms import MotRRsize, MotRRandomFlip, MotRRandomCrop, MotNormalize, MotPad
from mmot.datasets.pipelines.formatting import MotCollect, MotDefaultFormatBundle, MotShow



class DetmmotDetection:
    def __init__(self, args, transform, image_set='train', version='le135', npy2rgb=False):
        ...
        # assert(args.append_crowd, f'mmot dataset doesnot support crowdhuman dataset')
...
def build_transform(args, image_set):
    mmot_train = make_transforms_for_mmot_rgb('train', args)
    mmot_val = make_transforms_for_mmot_rgb('val', args)
...
def build(image_set, args):
    ...
    if image_set == 'train':
        dataset = DetmmotDetection(args, transform=transform, npy2rgb=args.npy2rgb)
    if image_set == 'val':
        dataset = DetmmotDetection(args, transform=transform, npy2rgb=args.npy2rgb)
    return dataset

def make_transforms_for_mmot_rgb(image_set, args=None):
    ...
    if image_set == 'train':
        return MotCompose([
            ...
            # MotShow(save_path='/data/users/litianhao/mmot_code/workdir/motrv2/debug', version='le135', mean=mean, std=std, img_name_tail='final', show_proposals=True, to_bgr=False),
    ...
    #         MotShow(save_path='/data/users/litianhao/mmot_code/workdir/debug/MotShow',to_bgr=False, img_name_tail='ori', show_proposals=True),
    ...
    #                 MotRRsize(multiscale_mode='value', img_scale=scales, bbox_clip_border=False),]
    #             ]),     
    #         MotShow(save_path='/data/users/litianhao/mmot_code/workdir/debug/MotShow',to_bgr=False, img_name_tail='randomcrop', show_proposals=True),     
             
    #         MotNormalize(mean=[66.04, 69.87, 61.45], std=[36.46, 35.70, 34.93]),
    #         MotPad(size_divisor=32),

    #         MotShow(save_path='/data/users/litianhao/mmot_code/workdir/debug/MotShow', version='le135', mean=[66.04, 69.87, 61.45], std=[36.46, 35.70, 34.93], img_name_tail='final', show_proposals=True),
