...
class mmot_8ch(MOTDataset):
    def __init__(self, config: dict, split: str, transform, version='le135'):
        super(mmot_8ch, self).__init__(config=config, split=split, transform=transform)
        ...
        self.split_dir = os.path.join(config["DATA_ROOT"], split, "npy")
        assert os.path.exists(self.split_dir), f"Dir {self.split_dir} is not exist."
        self.labels_dir = os.path.join(config["DATA_ROOT"], split, "mot")
...
def build(config: dict, split: str):
    if split == "train":
        ...
        return mmot_8ch(config=config, split=split, transform=transforms_for_eval())
    else:
