...
import numpy as np
import glob as gb
from mmot.mmlab.hs_mmrotate import obb2poly_np, poly2obb_np

from mmot.datasets.pipelines.compose import MotCompose, MotRandomChoice
from mmot.datasets.pipelines.channel import MotrToMmrotate, MmrotateToMotr, MmrotateToMotip, MotipToMmrotate
...
class MOTDataset(Dataset):
    ...
    def __init__(self, config: dict):
        ...
        self.transforms = self.get_transforms_mmot(config=config)
    ...
    def __getitem__(self, item):
        ...
        data_info = self.get_multi_frames_mmot(dataset=dataset, split=split, seq=seq, frames=frames_idx)
    ...
    def get_multi_frames_mmot(self, dataset: str, split: str, seq: str, frames: list[int]):
        return [self.get_single_frame_mmot(dataset=dataset, split=split, seq=seq, frame=frame) for frame in frames]

    def get_single_frame_mmot(self, dataset: str, split: str, seq: str, frame: int):
    ...
    @staticmethod
    def get_transforms_mmot(config: dict):
